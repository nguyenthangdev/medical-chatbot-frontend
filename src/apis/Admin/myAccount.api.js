...
export const fetchMyAccountAPI = async () => {
  ...
  const response = await authorizedAxiosInstance.get(`${API_ROOT}/api/v1/my-account`);
...
export const updateMyAccountAPI = async (data) => {
  ...
  const response = await authorizedAxiosInstance.patch(`${API_ROOT}/api/v1/my-account`, data);
  return response.data;
};