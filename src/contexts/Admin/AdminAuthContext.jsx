...
import { fetchMyProfileAPI } from '../../apis/Admin/myProfile.api';
...
export const AdminAuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        const res = await fetchMyProfileAPI();
        if (res && res.user) {
          setUser(res.user);
        } else {
          setUser(null);
...
export const useAuth = () => {
  ...
  if (!context) throw new Error('useAuth phải dùng bên trong AdminAuthProvider');
  return context;
};
