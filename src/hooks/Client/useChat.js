...
export const useChat = (userId, onChatUpdated) => {
  ...
  const sendMessage = useCallback(async (message, selectedModel = 'qwen') => {
    ...
    if (!message.trim() || loading || isLimitReached) return
    setError(null)
  ...
  }, [userId, loading, conversationId, onChatUpdated, isLimitReached]) 
  ...
  return { messages, loading, error, conversationId, isLimitReached, sendMessage, loadConversation, clearChat }
}